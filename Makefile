...
DOCKER_IMAGE_NAME=lispmeister/rpi-micropython
...
test: ## Test execution of Python
	docker run --rm $(DOCKER_IMAGE_TAGNAME) -c 'print("Success.")'

version: ## Print Python version installed
	docker run --rm $(DOCKER_IMAGE_TAGNAME) -c 'import sys; print(sys.version)'
